...
void Minimap::draw(FontAtlas& font, const std::string& buffer,
    ...
    while (lineStart <= buffer.size()) {
        ...
        if (y > windowH) break;
        // draw a thin colored block per line instead of actual text for perf
        ...
        if (lineW > 0) {
            ...
            addRect(miniTextX, blockY, miniTextX + blockW, blockY + blockH,
                    0.45f, 0.45f, 0.50f, 0.5f);
            glBindVertexArray(gl_vao());
            glBindBuffer(GL_ARRAY_BUFFER, gl_vbo());
            glBufferData(GL_ARRAY_BUFFER, verts.size() * sizeof(float), verts.data(), GL_DYNAMIC_DRAW);
            glBindTexture(GL_TEXTURE_2D, 0);
    ...
    // viewport highlight — highlight the currently visible region
    // assume scrollOffset=0 for now; will wire properly later
...
bool Minimap::handleClick(float mx, float my, float& scrollRatio) {
    // click within minimap area — calculate scroll position
    scrollRatio = 0.f; // will wire with scroll later
    return false;
}
